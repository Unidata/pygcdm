...
def test_interpret_spec():
    ...
    sizes = [1, 720, 1440]
    strides = [1, 1, 1]
    assert section == grpc_msg.Section(ranges=[grpc_msg.Range(size=s, stride=st) for s,st in zip(sizes, strides)])
    ...
    assert section == grpc_msg.Section(ranges=[grpc_msg.Range(size=s, stride=st, start=sa) for s,st,sa in zip(sizes, strides, starts)])
